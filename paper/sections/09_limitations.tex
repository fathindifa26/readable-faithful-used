...
\begin{itemize}
  ...
  \item \textbf{Six types, not a sweep.} The dissociation claim rests on
        six attribute types---a purposive sample of the 66 possible pairs
        among our twelve attributes (Sec.~\ref{sec:setup},
        Appendix~\ref{app:data}). That is enough for two unambiguous
        counterexamples, but not enough to generalize to intersections
        outside this set.
  \item \textbf{Fidelity and causality are measured in different
        contexts.} The two sides use different input contexts
        (identity-only vs.\ QA), and the map is measurably weaker in the
        QA context (Sec.~\ref{sec:sweep}); the fidelity--causality
        correlation is unchanged when re-computed on the QA-context
        ruler, but our headline fidelity numbers describe an
        identity-only prompt and should be read as such.
  \item \textbf{Single-layer patches only.} Because every patch is
        single-layer, redundant encoding across layers remains an
        alternative explanation for a null result that we have only
        partially ruled out (Sec.~\ref{sec:sweep}, caveat v).
  \item \textbf{\starhead{} discovery provenance.} The head was first
        noticed in per-type top-10 lists before being fixed for the
        all-type test. Sec.~\ref{sec:crossmodel} supplies the
        second-model check (L33\,H9, selection-free on five types); a
...
outputs for racial groups are the least trustworthy where
