...
\begin{itemize}
  ...
  \item \textbf{Correlational map, partial causal probe.} RSA fidelity is
        ...
        fidelity--causality relationship with precision. The two sides
        are also measured in different input contexts (identity-only
        vs.\ QA), and the map is measurably weaker in the QA context
        (Sec.~\ref{sec:sweep}); the fidelity--causality correlation is
        unchanged when re-computed on the QA-context ruler, but our
        headline fidelity numbers describe an identity-only prompt and
        should be read as such. And because every patch is
