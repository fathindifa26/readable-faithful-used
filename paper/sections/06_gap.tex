...
\paragraph{The aggregate hides a ninefold spread.}
...
demographic block; the patch is applied \emph{at that identity token},
so it propagates to every subsequent token; all 32 heads of layer 11 are
patched jointly; and questions are selected for maximal real inter-group
disagreement. Across the three types tested here the result is
\textbf{graded}, and the gradation
appears to line up with each type's fidelity from Sec.~\ref{sec:act2}.
Sec.~\ref{sec:sweep} extends the experiment to all six types and shows
that this alignment does \emph{not} generalise. The three-type result is
the strongest single demonstration that the effect exists at all:
...
\begin{enumerate}
  \item \textbf{The highest-fidelity type shows a real, correctly-directed
        ...
        hypothesis, though computed on the same pairs.
  \item \textbf{The lowest-fidelity type shows no correct effect, and a
        ...
        control in all 12 pairs; pair-level $t=-5.2$,
...
\paragraph{What survives cluster-robust inference.}
Figure~\ref{fig:sweep} shows the descriptive item-level picture; the
pair-level statistics behind Table~\ref{tab:sweep} are deliberately
conservative (12 clusters per type), and they prune it sharply. No type survives the pair-level max-statistic
correction (best: RACE$\times$POLIDEOLOGY, $p=0.066$): with 12 pairs one
cannot both search 32 layers and certify the winner. The a-priori test at L11 (no layer selection; L11 was fixed
by Sec.~\ref{sec:act2}) is cleaner:
...
layer~1---with one asymmetry the table itself does not show:
...
$-0.09$). With $n{=}6$ these correlations are weakly
...
generalise; on the six-type evidence it is a coincidence.
...
\paragraph{Five caveats.} (i) Absolute effects are small
...
\paragraph{Reading the map beats listening to the mouth, and not by
calibration alone.}
...
discrimination: at that resolution the signal is near zero. The
